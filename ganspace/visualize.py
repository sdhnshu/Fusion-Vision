...
def make_gif(imgs, duration_secs, outname):
    ...
    head.save(outname, format='GIF', append_images=tail,
...
if __name__ == '__main__':
    ...
    inst = get_instrumented_model(args.model, args.output_class,
                                  layer_key, device, use_w=args.use_w)
    model = inst.model
    ...
    random_dirs_act = torch.from_numpy(get_random_dirs(
        n_comp, np.prod(sample_shape)).reshape(-1, *sample_shape)).to(device)
    random_dirs_z = torch.from_numpy(get_random_dirs(n_comp, np.prod(
        inst.input_shape)).reshape(-1, *latent_shape)).to(device)
