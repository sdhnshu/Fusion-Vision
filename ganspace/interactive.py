...
def load_named_components(path, class_name):
    ...
    comp_dict = {k: [] for k in ['X_comp', 'Z_comp', 'X_stdev', 'Z_stdev',
                                 'names', 'types', 'layer_names', 'ranges', 'latent_types']}
...
def setup_model():
    ...
    # Load model
    inst = get_instrumented_model(model_name, class_name, layer_name,
                                  torch.device('cuda'), use_w=args.use_w)
    model = inst.model
...
def setup_ui():
    ...
    for i in range(N_COMPONENTS):
        ...
        if not use_named_latents:
            tk.Button(inner, text=f"Save", command=partial(
                export_direction, i, inner)).pack(fill=tk.Y, side=tk.RIGHT)
        inner.pack(fill=tk.X)

    outer_frame.pack(fill="both", expand=True, pady=0)

    tk.Button(toolbar, text="Reset", command=reset_sliders).pack(
        anchor=tk.CENTER, fill=tk.X, padx=4, pady=4)
    ...
    tk.Entry(frame, textvariable=ui_state.outclass).pack(
        fill="x", side="right", expand=True, padx=5)
    ...
    tk.Entry(frame, textvariable=ui_state.random_seed, width=12).pack(
        fill="x", side="left", expand=True, padx=2)
    tk.Button(frame, text="Update", command=update_seed).pack(fill="y", side="right", padx=3)
    frame.pack(fill=tk.X, pady=3)

    # Get new latent or new components
    tk.Button(toolbar, text="Resample latent", command=partial(resample_latent,
                                                               None, False)).pack(anchor=tk.CENTER, fill=tk.X, padx=4, pady=4)
        ...
        with torch.no_grad():
            ...
            for c in range(n_comp):
                ...
                if edit_mode in ['latent', 'both']:
                    ...
                    edit_range = components.ranges[c] if use_named_latents else (
